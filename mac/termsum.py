...
class term_sum:
    def __init__(self, term_info):
        ...
        #self.info = self.cq.passthru_generate_term()
        self.info = term_info
        self.term_sum = []

        # Extract only the data needed for term summary.
        for i in self.info:
            student = []
            ...
            self.term_sum.append(student)

    def create_txt_file(self):
        # Write txt file.
        file = open('Term_Summary.txt', 'w')
        ...
        # value in term_sum list exceeds it.
        for i in range(len(self.term_sum)):
            for j in range(len(self.term_sum[i]) - 1):
                if (len(str(self.term_sum[i][j])) > max_sizes[j]):
                    max_sizes[j] = len(str(self.term_sum[i][j]))
        ...
        # Increment value in max_sizes list by 4 if length of the string rep
        # of a value in term_sum list exceeds it.
        for i in range(len(self.term_sum)):
            for j in range(len(self.term_sum[i])):
                if (j < len(self.term_sum[i])-1):
                    word_size = len(str(self.term_sum[i][j]))
        ...
        for i in range(len(self.term_sum)):
            for j in range(len(self.term_sum[i])):
                if (j < len(self.term_sum[i])-1):
                    ...
                    spaces = " " * dif
                    file.write(str(self.term_sum[i][j]) + spaces)
                else:
                    file.write(str(self.term_sum[i][j]) + '\n')
