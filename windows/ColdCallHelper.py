...
Layout -
    Small GUI - opens at the top of screen in a wide but short window good for using with other apps like powerpoint open
        ...
        Menubar -
            ...
            Export -
                ...
                1 - Problem (Alt+1) - used to remeber student was absent or caused a concern
                2 - Problem (Alt+2) - used to remeber student was absent or caused a concern
                3 - Problem (Alt+3) - used to remeber student was absent or caused a concern
    ...
    *if the FERPA flag is true for a student in roster.tsv, the nickname will apear and nothing else so make sure once is present if the flag is checked or else 'unnamed' names will apear
...
Class/Def Descriptions -
    ...
    Application class- sets small sreen size mode to on, turns on pronounciation/nickname when bigscreen is on, intializes class_Queue and helpPopup
        ...
        def thirdProblem() - bind ctrl+3 - used to make a problem mark the student when there is a problem or absence
            calls on the queue to update the postion and mark in the datarepo
        def firstFollowup() - bind alt+1 or alt+spacebar - used to make a problem mark student when you wish to remember a good question or to follow up
            calls on the queue updates the student labels and makes a followup mark in the datarepo
        def secondFollowup() - bind alt+2 - used to make a problem mark student when you wish to remember a good question or to follow up
                calls on the queue updates the student labels and makes a followup mark in the datarepo
        def thirdFollowup() - bind alt+3 - used to make a problem mark student when you wish to remember a good question or to follow up
                calls on the queue updates the student labels and makes a followup mark in the datarepo
...
class Application:
    ...
    if students.data_repo.begin() == "Ready":
        if not students.load_queue():
            students.update_roster_and_queue()
            #students.fill_both_bags_from_roster()
            students.save_queue()
        else:
            err = "No files set up, open menu to help user."
    ...
    def __init__(self,master):
        ...
        self.selectionmenu.add_separator()
        self.selectionmenu.add_command(label="1 - Problem (Alt+1)", command=self.firstProblem)
        self.selectionmenu.add_command(label="2 - Problem (Alt+2)", command=self.secondProblem)
        self.selectionmenu.add_command(label="3 - Problem (Alt+3)", command=self.thirdProblem)
        ...
        master.bind('<Alt-Key-1>', self.firstProblem)
        master.bind('<Alt-space>', self.firstProblem)
        master.bind('<Alt-Key-2>', self.secondProblem)
        master.bind('<Alt-Key-3>', self.thirdProblem)
